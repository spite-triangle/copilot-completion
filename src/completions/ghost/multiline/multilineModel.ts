...
const commentMap: { [key: string]: string[] } = {
    ...
    javascriptreact: ['//'], vue: ['//', '-->'], php: ['//', '#'],
    dart: ['//'], go: ['//'], cpp: ['//'], scss: ['//'], csharp: ['//'],
    java: ['//'], c: ['//'], rust: ['//'], python: ['#'],
    markdown: ['#', '-->'], css: ['*/'],
};

const languageMap: { [key: string]: number } = {
    javascript: 1, javascriptreact: 2, typescript: 3, typescriptreact: 4,
    python: 5, go: 6, ruby: 7,
};
...
export class PromptFeatures {
    ...
    private firstAndLast(text: string): string[] {
        const lines = text.split('\n');
        const numLines = lines.length;
        const firstLine = lines[0];
        let lastLine = lines[numLines - 1];
        if (lastLine === '' && numLines > 1) { lastLine = lines[numLines - 2]; }
...
interface PromptLike { prefix: string; suffix: string }

export function requestMultilineScore(prompt: PromptLike, language: string): number {
    const features = new MultilineModelFeatures(prompt.prefix, prompt.suffix, language).constructFeatures();
